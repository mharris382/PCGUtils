...
bool FPCGUtilsFractureRoundTripTest::RunTest(const FString&)
{
	...
	TestEqual(TEXT("One geometry bone before fracture"), CountPieces(Collection), 1);
	...
	const int32 FracturedBones = CountPieces(Fractured);
	...
	const int32 RemainingBones = CountPieces(Pruned);
...
bool FPCGUtilsFractureSurfaceAttributesTest::RunTest(const FString&)
{
	...
	{
		TArray<int32> Bones;
		PCGUtilsGeometryCollectionHierarchy::GatherPieces(Fractured->GetCollection(), Bones);
	...
	{
		TArray<int32> Bones;
		PCGUtilsGeometryCollectionHierarchy::GatherPieces(Damaged->GetCollection(), Bones);
		TArray<FTransform> Globals;
...
bool FPCGUtilsFractureBoneSurfaceTest::RunTest(const FString&)
{
	...
	{
		TArray<int32> Bones;
		PCGUtilsGeometryCollectionHierarchy::GatherPieces(Unfractured->GetCollection(), Bones);
...
bool FPCGUtilsUniformVoronoiFractureTest::RunTest(const FString&)
{
	...
	const int32 Pieces = CountPieces(Fractured);
	...
	if (TestNotNull(TEXT("Coarse fracture"), Coarse) && TestNotNull(TEXT("Fine fracture"), Fine))
	{
		TestTrue(TEXT("More sites yields more pieces"),
			CountPieces(Fine) > CountPieces(Coarse));
	}
...
bool FPCGUtilsFractureTransformSpaceTest::RunTest(const FString&)
{
	...
	TestEqual(TEXT("Offsetting the source changes nothing structurally"),
		CountPieces(Offsetted), CountPieces(Centered));
