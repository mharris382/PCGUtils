...
{
	return LOCTEXT("NodeTitle", "DEPRECATED: Create Primitive");
}

FText UPCGCreatePrimitiveSettings::GetNodeTooltipText() const
{
	return LOCTEXT("NodeTooltip",
		"DEPRECATED - use DynMesh|Realize Builders with Builder nodes instead. Generates a Geometry Script "
		"primitive as Dynamic Mesh data. In legacy mode, one copy of the inline primitive is appended per "
		"point on the Seeds pin. In Builder mode (bUseLegacyMode disabled), the primitives come from Builder "
		"nodes on the Builders pin instead - which is exactly what DynMesh|Realize Builders now does, "
		"discoverably.");
}
...
{
	// One implementation, shared with DynMesh|Realize Builders, so the deprecated path cannot drift from it.
	return PCGUtilsDynMeshBuilderRealization::Realize(
		Context, Settings->bConvertSeedsToLocalSpace, Settings->OutputMode,
		LOCTEXT("LegacyNodeName", "Create Primitive"));
}
